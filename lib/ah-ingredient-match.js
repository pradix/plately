  ...
  try {
    for (const part of parts) {
      ...
      if (part === "gember" || part === "ginger") {
        ...
        const fruitCue =
          /\b(aardbei|strawberry|framboos|raspberry|appel|apple|mango|ananas|pineapple|sinaasappel|orange)\b/i.test(titleLow);
      ...
      if (part === "citroengras" || part === "lemongrass" || part === "sereh") {
        ...
        if (/\b(bruiswater|water|drank|sap|sapje|shot|smoothie|juice)\b/i.test(title)) return false;
      }
      ...
      if (part === "kip") {
        ...
        if (/\b(kippen?bouillon|bouillon)\b/i.test(title) && /\bkip(pen)?\b/i.test(title)) return false;

  ...
  try {
    ...
    for (const part of parts) {
      ...
      if (part === "mango") {
        ...
        // Multi-fruit blends / cold-pressed juices (bijv. "Ginger mango appel gember"):
        // als de titel naast mango ook andere vruchten/groenten noemt = composietdrank.
        if (/\b(ginger|verse\s+(?:appel|ananas|wortel|sinaasappel)|appel\s+\w+\s+gember)\b/i.test(title)) return false;
        if (/\b(appel|ananas|gember|wortel|sinaasappel|peer)\b/i.test(title) &&
            /\b(biologisch|verse?|cold\s*press|pressed|blend)\b/i.test(title)) return false;
        if (looksProcessed) return false;
      }
      if (part === "ananas") {

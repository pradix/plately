...
function ingredientTermMatchesProductTitle(term, productTitle) {
  ...
  try {
    ...
    for (const part of parts) {
      ...
      // Detecteer ook: leeftijdsindicatoren (4+, 6m, 8m06, 12m+) en samengestelde hapjes.
      if (/\b(knijpfruit|knijpzakje|babyvoeding|babyhapje|baby\s*maaltijd|fruitmoes|fruitpap|groentehapje|fruitpuree\s+baby|peuterkoek|peuterreep)\b/i.test(title)) return false;
      if (/\b\d+\s*m\+?\b/i.test(title) && /\b(hapje|pap|moes|puree|brood|groente|fruit|maaltijd|biologisch)\b/i.test(title)) return false;

...
function ingredientTermMatchesProductTitle(term, productTitle) {
  ...
  try {
    ...
    for (const part of parts) {
      ...
      if (/\b(knijpfruit|knijpzakje|babyvoeding|babyhapje|baby\s*maaltijd|fruitmoes|fruitpap)\b/i.test(title)) return false;
      ...
      if (part === "knoflook") {
        ...
        const KNOFLOOK_BLOCKER =
          /\b(roomkaas|kaas|saus|dressing|mayo|mayonaise|aioli|boter|kruidenboter|olie|azijn|vinaigrette|marinade|mix|kruidenmix|kruiden|pasta|puree|crouton|croutons|chips|dip|spread|hummus|tapenades?|pers|press|snijder|rasp)\b/i;
        if (KNOFLOOK_BLOCKER.test(title)) return false;
        // Composita: "knoflooksaus", "knoflookboter", "knoflookmayo", "knoflookolie", etc.
        if (/knoflook(saus|boter|mayo|mayonaise|olie|azijn|roomkaas|kaas|pasta|puree|croutons?|chips|dip|spread|pers|press|snijder|rasp|kruidenmix|mix)/i.test(title)) return false;

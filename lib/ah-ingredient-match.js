...
function ingredientTermMatchesProductTitle(term, productTitle) {
  ...
  const VERS_BASIS = new Set([
    ...
    "zout","azijn","olie","rijst","quinoa","gist","zetmeel","maizena","honing","mosterd",
    ...
    "kip","vlees","gehakt","spek","ham","kaas","feta","kwark","yoghurt",
    "peper","kruiden","oregano","basilicum","tijm","rozemarijn","koriander",
    "peterselie","munt","dille","bieslook","dragon",
    "citroen","citroenen","limoen","limoenen",
    "appel","appels","appelen","peer","peren",
    "banaan","bananen","aardbei","aardbeien","bosbes","bosbessen","bes","bessen","framboos","frambozen",
  ...
  const VERS_FRUIT = new Set([
    ...
    "bosbes","bosbessen","bes","bessen","framboos","frambozen",
  ...
  // Multi-fruit blend detectie: als het ingrediënt één vrucht is maar de producttitel
  // meerdere vruchten/groenten combineert met een vers/biologisch-aanduiding, is het
  // vrijwel zeker een sap, smoothie of cold-press blend — geen losse vrucht.
  // Wordt vóór de per-part loop geëvalueerd zodat het voor alle fruitsoorten geldt.
  const searchedFruitInTitle = parts.some((p) => VERS_FRUIT.has(p));
  if (searchedFruitInTitle && !ingredientWantsDrink && !ingredientWantsJam) {
    const FRUIT_BLEND_WORDS = [
      "appel","peer","mango","ananas","banaan","aardbei","aardbeien","sinaasappel",
      "perzik","druif","kiwi","meloen","watermeloen","abrikoos","kers","framboos","frambozen",
      "bosbes","bosbessen","bes","bessen",
      "gember","wortel","biet","spinazie","komkommer","selderij","citroen","limoen",
    ];
    const BLEND_SIGNAL = /\b(biologisch|verse?|cold[\s-]?press(?:ed)?|pressed|blend|mix|ginger)\b/i;
    const otherFruitCount = FRUIT_BLEND_WORDS.filter(
      (fw) => !parts.includes(fw) && new RegExp(`\\b${fw}\\b`, "i").test(title)
    ).length;
    if (otherFruitCount >= 2 && BLEND_SIGNAL.test(title)) return false;
    if (otherFruitCount >= 3) return false; // 3+ andere ingrediënten = zeker een blend
  }
  ...
  try {
    ...
    for (const part of parts) {
      ...
      const looksProcessed =
        /\b(soep|saus|dressing|marinade|pasta|puree|passata|ketchup|dip|spread|chips|snack|toast|toastjes|crackers?|rijstwafels?|wafels?|koek|koekjes|biscuit|shuttles?|smaak|kwark|yoghurt|protein|börek|borek)\b/i.test(
          title
        );
      ...
      if (part === "paprika") {
        ...
        if (/\b(gegrilde?|geroosterde?|antipasti|op\s+zuur|in\s+olie|pot|reepjes)\b/i.test(title)) return false;
        if (looksProcessed) return false;
      }
      if (part === "kruiden") {
        // Kruidenmix/-strooier mag, maar geen tomatensaus of maaltijd met kruiden als smaak.
        if (/\b(tomatensaus|saus|basis|pasta|maaltijd|dressing|marinade|soep)\b/i.test(title)) return false;
        if (/\b(kruidenmix|italiaanse\s+kruiden|provencaalse\s+kruiden|provençaalse\s+kruiden|strooier|gedroogd|potje)\b/i.test(title)) continue;
      }
      ...
      if (part === "olie") {
        // Olie ≠ producten "in olie" of ingelegde antipasti.
        if (/\bin\s+olie\b/i.test(title)) return false;
        if (/\b(artisjok|artisjokken|olijven?|antipasti|gegrilde?|paprika)\b/i.test(title) && !/\b(olijfolie|zonnebloemolie|arachideolie|sesamolie|kokosolie|slaolie|bakolie)\b/i.test(title)) return false;
        if (/\b(olijfolie|zonnebloemolie|arachideolie|sesamolie|kokosolie|slaolie|bakolie)\b/i.test(title)) continue;
      }
      if (part === "quinoa") {
        // Quinoa als graan/pseudograan, niet rijstwafels of repen met quinoa.
        if (/\b(rijstwafels?|wafels?|crackers?|chips|repen?|reep|granola|muesli|ontbijt|snack)\b/i.test(title)) return false;
      }
      if (part === "rijst") {
      ...
      if (part === "kaas") {
        ...
        if (/\b(grissini|broodstengels?|crackers?|toastjes?|koekjes?|biscuits?|chips|snack|minikoek|mini\s*koek)\b/i.test(title)) return false;
      }
      if (part === "feta") {
        // Feta als kaasblok/verpakking, niet bereide snacks of gerechten met feta.
        if (/\b(börek|borek|spinazie|snack|pizza|wraps?|quiche|salade|broodje|filodeeg|bladerdeeg|hapjes?)\b/i.test(title)) return false;
      }
      if (part === "framboos" || part === "frambozen") {
        if (/\b(kwark|yoghurt|protein|smaak|drank|sap|limonade|siroop|frisdrank|bubblegum|ijs|jam)\b/i.test(title)) return false;
        if (/\bframbozen\b/i.test(title)) continue;
      }
      if (part === "bes" || part === "bessen" || part === "bosbes" || part === "bosbessen") {
        if (/\b(kwark|yoghurt|protein|smaak|drank|sap|limonade|siroop|frisdrank|bubblegum|ijs|jam)\b/i.test(title)) return false;
        if (/\b(blauwe\s+bessen|bosbessen|bessen)\b/i.test(title)) continue;
      }
      if (part === "wortel" || part === "wortelen") {
        ...
        // Wortel als smaak in wraps/tortilla/chips/snacks ≠ verse wortel.
        if (/\b(wraps?|tortilla|chips|crackers?|biscuits?|koekjes?|snack)\b/i.test(title)) return false;
      ...
      if (part === "mango") {
        ...
        if (looksProcessed) return false;
        // Multi-fruit blend wordt globaal afgevangen vóór de per-part loop.
      }
      if (part === "ananas") {
      ...
      if (part === "ei" || part === "eieren") {
        ...
        if (/\b(spaghetti|pasta\b|penne|lasagne|macaroni|nasi\b|bami\b|noedels|noodles|ramen\b|stamppot|frittata|omelet\b|quiche|wraps?\b|tortilla\b|curry\b|wokmaaltijd|rijstschotel)\b/i.test(title)) return false;
...
function buildIngredientMatchTerms(rawIngredient, normalizedBase) {
  ...
  if (/\b(?:blauwe\s+bes|blauwe\s+bessen|bosbes|bosbessen)\b/.test(base) || /\b(?:blauwe\s+bes|blauwe\s+bessen|bosbes|bosbessen)\b/.test(raw)) {
    terms.add("blauwe bes");
    terms.add("blauwe bessen");
    terms.add("bosbes");
    terms.add("bosbessen");
    terms.add("bessen");
  }
  if (/\bframbo(?:os|zen)\b/.test(base) || /\bframbo(?:os|zen)\b/.test(raw)) {
    terms.add("framboos");
    terms.add("frambozen");
  }
  if (base === "quinoa" || /\bquinoa\b/.test(raw)) {
    terms.add("quinoa");
  }
  if (/\bitaliaanse\s+kruiden\b/.test(base) || /\bitaliaanse\s+kruiden\b/.test(raw)) {
    terms.add("italiaanse kruiden");
    terms.add("kruiden");
  }

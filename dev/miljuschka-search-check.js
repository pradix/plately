/**
 * Dev smoke script: hit local channel-search for Miljuschka (prints first 5 hits).
 *
 * Usage:
 *   node dev/miljuschka-search-check.js pasta 5055
 */
const query = String(process.argv[2] || "pasta").trim();
const port = Number.parseInt(String(process.argv[3] || "5055"), 10) || 5055;

async function main() {
  const url = `http://localhost:${port}/api/channel-search?q=${encodeURIComponent(query)}&channels=ch-mj`;
